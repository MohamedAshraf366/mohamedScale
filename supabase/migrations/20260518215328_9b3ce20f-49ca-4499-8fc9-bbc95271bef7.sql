...
-- =========================================================
-- Pricing settings: admin-managed system defaults
-- =========================================================
...
DROP POLICY IF EXISTS "Anyone authenticated can read pricing_settings" ON public.pricing_settings;
CREATE POLICY "Anyone authenticated can read pricing_settings"
...
-- Seed the singleton (no margin yet — admin must set it explicitly)
INSERT INTO public.pricing_settings (id, default_margin_pct)
VALUES (true, NULL)
ON CONFLICT (id) DO NOTHING;

-- =========================================================
-- compute_quotation_totals (Phase 3 corrections)
-- =========================================================
...
BEGIN
  ...
  -- System default margin from admin-managed pricing_settings (NOT metadata.global_margin)
  ...
  LOOP
    ...
    ELSE
      ...
      ELSE
        -- STRICT: status=approved AND is_current=true AND (price_valid_until null OR future)
        ...
        ELSE
          -- Soft fallback: most recent approved (any currency/validity)
          SELECT sm.id, sm.unit_price, sm.moq, sm.supplier_quote_id, sm.price_valid_until, sm.status
            INTO v_supplier_material_id, v_supplier_unit_price, v_supplier_moq,
                 v_supplier_quote_id, v_price_valid_until, v_supplier_status
            FROM public.supplier_materials sm
    ...
    -- Margin hierarchy: line override -> (material override: not yet implemented) -> subcategory -> system default
    -- NEVER silently fall to 0: emit warning if no system default exists
    ...
    BEGIN
      ...
      IF v_line.raw_unit IS NULL THEN
        v_trace := v_trace || jsonb_build_object(
          'landed_unit_cost', NULL, 'final_unit_price', NULL, 'final_line_total', NULL
        );
        UPDATE public.quotation_items
      ...
      ELSE
        ...
        UPDATE public.quotation_items
           SET unit_price=v_final_unit, delivery_price=v_dpu, line_total=v_line_total,
