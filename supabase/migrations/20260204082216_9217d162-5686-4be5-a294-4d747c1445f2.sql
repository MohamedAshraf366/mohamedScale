-- Note: Regions were already inserted in previous migration
-- Now create 30 zones for Riyadh in a 5x6 grid
-- Riyadh approximate bounds: lat 24.4-25.0, lng 46.3-47.1
...
BEGIN
  ...
  IF riyadh_id IS NULL THEN
    RAISE NOTICE 'Riyadh region not found, skipping zone creation';
    RETURN;
  END IF;
  ...
  FOR row_idx IN 0..5 LOOP
    FOR col_idx IN 0..4 LOOP
      ...
      -- Format zone code as RUH.001, RUH.002, etc.
