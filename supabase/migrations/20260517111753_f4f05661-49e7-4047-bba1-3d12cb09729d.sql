...
-- 1. Add 'rejected' to enum (so Domain editor can record explicit rejections; resolver ignores it)
ALTER TYPE public.supplier_selection_role ADD VALUE IF NOT EXISTS 'rejected';

-- 2. Rebuild resolve_supplier with landed-price backup tiebreak and usable-supplier filter
  ...
  FOREACH v_scope IN ARRAY v_scopes LOOP
    FOREACH v_role_try IN ARRAY v_role_order LOOP
      ...
      IF v_role_try = 'backup' THEN
        -- Cheapest active backup by landed price for the requested zone.
        -- Requires a usable current/valid quote and delivery coverage for the zone.
        WITH cand AS (
