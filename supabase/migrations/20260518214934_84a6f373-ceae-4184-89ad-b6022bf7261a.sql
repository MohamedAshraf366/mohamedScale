...
BEGIN
  ...
  -- PASS 1: resolve supplier + supplier_material + delivery_rate + moq + margin per line.
  --         Store staging values in pricing_trace; clear unit_price/line_total temporarily.
  FOR v_line IN
  ...
  LOOP
    ...
    v_warnings := '[]'::jsonb;
    v_material_code := NULL; v_subcategory_id := NULL;
    ...
    -- Margin hierarchy: line override -> material override (not yet schema) -> subcategory -> system default
    ...
    v_stage := jsonb_build_object(
      ...
      'supplier_quote_id', v_supplier_material_id,
  ...
  -- PASS 2: compute group aggregates from staged trace, then write final pricing per line.
  FOR v_line IN
    WITH lines AS (
      SELECT qi.id AS line_id,
             qi.quantity,
    ...
    SELECT l.line_id, l.quantity, l.raw_unit, l.margin_pct, l.trace_in,
      ...
      LEFT JOIN group_calc g
             ON g.supplier_id = l.supplier_id
            AND g.delivery_rate_id = l.delivery_rate_id
  LOOP
    DECLARE
    ...
    BEGIN
      ...
      IF v_line.raw_unit IS NULL THEN
        ...
        UPDATE public.quotation_items
           SET unit_price = NULL, delivery_price = NULL, line_total = NULL,
  ...
  -- Header totals: subtotal from line_total (already embeds delivery+margin for materials,
  -- and manual values for custom/addon). delivery_total = sum of unique group delivery costs.
